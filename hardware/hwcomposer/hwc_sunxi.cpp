...
#if 0
    if(zOrder != 0 && (layer->displayFrame.right - layer->displayFrame.left) >= 1920)
    {
        return 0;
    }
#endif
...
static int _hwcdev_setup_layer(SUNXI_hwcdev_context_t *ctx, __disp_layer_info_t *layer_info, SUNXI_hwcdev_pipe_t *psPipe, size_t disp)
{
    ...
    //if(psPipe->assignedLayerZOrder == 0)
    {
        ...
        if((layer_info->src_win.x + layer_info->src_win.width) > (__u32) handle->width)
        {
        ...
        if((layer_info->src_win.y + layer_info->src_win.height) > (__u32) handle->height)
        {
        ...
        if((layer_info->scn_win.x + layer_info->scn_win.width) > (__u32) scn_bound.right)
        {
        ...
        if((layer_info->scn_win.y + layer_info->scn_win.height) > (__u32) scn_bound.bottom)
        {
