					   ...
					   hwc_display_contents_1_t **displays)
{
    ...
    recalculate_pipe_assignments:
    	...
    	if (needFramebufferTargetLayer == 1)
    	{
    		...
    		{
    			ALOGV("%s: display[%d] FRAMEBUFFER_TARGET layer not last layer",
    			                        __func__, disp);
    			continue;
    		}
    		...
    		{
    			ALOGV("%s: display[%d] FRAMEBUFFER_TARGET layer failed to be assigned to pipe",
    			                        __func__, disp);
    	...
    	for(i = 0; i < psDisplay->numHwLayers-1; i++)
    	{
    		...
    		switch(psLayer->compositionType)
    		{
    			...
    			case HWC_OVERLAY:
    			{
    				...
    				for (pipeNo = 0; pipeNo < ctx->pipeCount[disp]; pipeNo++)
    				{
//    					if (!ctx->pipes[disp][pipeNo].pipeInUse &&
//                                hwcdev_try_to_assign_pipe(ctx, &ctx->pipes[disp][pipeNo], psLayer, i, disp)
...
#if 0
                        if (!ctx->pipes[disp][pipeNo].pipeInUse && assignmentSucceeded == 0)
                        {
