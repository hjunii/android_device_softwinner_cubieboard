...
omx_vdec::~omx_vdec()
{
    ...
    if(mDecodeFrameTotalCount!=0 && mConvertTotalCount!=0)
    {
        ...
        ALOGV("decode and convert statistics:\
...
OMX_ERRORTYPE omx_vdec::component_init(OMX_STRING name)
{
    ...
    m_sOutPortDef.format.video.eColorFormat = (OMX_COLOR_FORMATTYPE) HAL_PIXEL_FORMAT_YV12;
...
//	            m_sOutPortDef.format.video.nStride = m_sOutPortDef.format.video.nFrameWidth;
            ...
            //#if (defined(__CHIP_VERSION_F23) || defined(__CHIP_VERSION_F51) || defined(__CHIP_VERSION_F50))
                ...
                {
                    ALOGV("(f:%s, l:%d) original gpu buf align_width[%d], height[%d]mod8 = %d", __FUNCTION__, __LINE__, nGpuBufWidth, nGpuBufHeight, nGpuBufHeight%8);
...
//    write(m_cmddatapipe[1], &eCmd, sizeof(eCmd));
    ...
    ALOGV("(f:%s, l:%d) two threads exit!", __FUNCTION__, __LINE__);
...
static void* ComponentThread(void* pThreadData)
{
    ...
    while (1)
    {
        ...
        if (FD_ISSET(pSelf->m_cmdpipe[0], &rfds))
        {
	        ...
	        if (cmd == SetState)
	        {
	        	ALOGV(" set state command, cmd = %d, cmddata = %d.", (int)cmd, (int)cmddata);
	                ...
	                switch ((OMX_STATETYPE)(cmddata))
	                {
		                ...
		                case OMX_StateLoaded:
                            if (pSelf->m_state == OMX_StateIdle || pSelf->m_state == OMX_StateWaitForResources)
                            {
                                ...
                                ALOGV("(f:%s, l:%d) wait for OMX_VdrvCommand_CloseVdecLib", __FUNCTION__, __LINE__);
                                sem_wait(&pSelf->m_vdrv_cmd_lock);
                                ALOGV("(f:%s, l:%d) wait for OMX_VdrvCommand_CloseVdecLib done!", __FUNCTION__, __LINE__);
		                    }
			                else
			                {
                        ...
                        case OMX_StateIdle:
		                    ...
		                    else
		                    {
			                    ...
			                    else    //OMX_StateLoaded -> OMX_StateIdle
			                    {
                                    ...
                                    ALOGV("(f:%s, l:%d) wait for OMX_VdrvCommand_PrepareVdecLib", __FUNCTION__, __LINE__);
                                    sem_wait(&pSelf->m_vdrv_cmd_lock);
                                    ALOGV("(f:%s, l:%d) wait for OMX_VdrvCommand_PrepareVdecLib done!", __FUNCTION__, __LINE__);
			                    }
	        ...
	        {
	        	ALOGV(" stop port command, cmddata = %d.", (int)cmddata);
	            // Stop Port(s)
	        ...
	        {
	        	ALOGV(" restart port command.pSelf->m_state[%d]", pSelf->m_state);
                // Restart Port(s)
...
//	            	pSelf->m_sOutPortDef.bEnabled = OMX_TRUE;
	        ...
	        else if (cmd == Flush)
	        {
                ...
                {
                    ALOGV(" flush all port! we reset decoder!");
                    post_message_to_vdrv(pSelf, OMX_VdrvCommand_FlushVdecLib);
                    sem_post(&pSelf->m_sem_vbs_input);
                    sem_post(&pSelf->m_sem_frame_output);
                    ALOGV("(f:%s, l:%d) wait for OMX_VdrvCommand_FlushVdecLib", __FUNCTION__, __LINE__);
                    sem_wait(&pSelf->m_vdrv_cmd_lock);
                    ALOGV("(f:%s, l:%d) wait for OMX_VdrvCommand_FlushVdecLib done!", __FUNCTION__, __LINE__);
                }
	            if (cmddata == 0x0 || (OMX_S32)cmddata == -1)
	            {
	        ...
	        else if (cmd == Stop)
	        {
                ...
                ALOGV("(f:%s, l:%d) wait for OMX_VdrvCommand_Stop", __FUNCTION__, __LINE__);
                sem_wait(&pSelf->m_vdrv_cmd_lock);
                ALOGV("(f:%s, l:%d) wait for OMX_VdrvCommand_Stop done!", __FUNCTION__, __LINE__);
  		        // Kill thread
 	            goto EXIT;
	        }
	        else if (cmd == FillBuf)
	        ...
	        else if (cmd == EmptyBuf)
	        {
                ...
                else
                {
                    if(pSelf->m_nInportPrevTimeStamp)
                    {
                        ...
                        {
                            ALOGV("pts jump [%lld]us, prev[%lld],cur[%lld],need reset vdeclib", nInterval, pSelf->m_nInportPrevTimeStamp, pTmpInBufHeader->nTimeStamp);
        	...
        	port_setting_match)
        {
       		...
       		while(0==pSelf->m_decoder->picture_ready(pSelf->m_decoder) && pSelf->m_sInBufList.nSizeOfList>0)
    		{
                ...
                if(pInBufHdr == NULL)
                {
                    ALOGV("(f:%s, l:%d) fatal error! pInBufHdr is NULL, check code!", __FUNCTION__, __LINE__);
                    //usleep(20*1000);
                	break;
                }
                ...
                if(0 == nRetSemGetValue)
                {
                    ...
                    {
                        //ALOGV("(f:%s, l:%d) Be careful, sema vbs_input[%d]!=0", __FUNCTION__, __LINE__, nSemVal);
                    }
                }
                else
                {
                ...
                if (pInBufHdr)
                {
                    if (pInBufHdr->nFlags & OMX_BUFFERFLAG_EOS)
                    {
                        ...
                        sem_post(&pSelf->m_sem_vbs_input);
                        sem_post(&pSelf->m_sem_frame_output);
                        ALOGV("(f:%s, l:%d) wait for OMX_VdrvCommand_EndOfStream", __FUNCTION__, __LINE__);
                        sem_wait(&pSelf->m_vdrv_cmd_lock);
                        ALOGV("(f:%s, l:%d) wait for OMX_VdrvCommand_EndOfStream done!", __FUNCTION__, __LINE__);

        	        	ALOGV(" set up eos flag.");
...
//                    tm1 = OMX_GetNowUs();
                    ...
                    {
//                        tm2 = OMX_GetNowUs();
//                        ALOGV("(f:%s, l:%d) no output frame, wait 40ms for msg, interval=[%lld]ms", __FUNCTION__, __LINE__, (tm2-tm1)/1000);
                    }
                    else
                    {
//                        tm3 = OMX_GetNowUs();
//                        ALOGV("(f:%s, l:%d) no output frame, wait <40ms for msg coming, interval=[%lld]ms", __FUNCTION__, __LINE__, (tm3-tm1)/1000);
                    }
                    continue;
            	...
            	{
            		//ALOGV("(omx_vdec, f:%s, l:%d) m_useAndroidBuffer==OMX_FALSE", __FUNCTION__, __LINE__);
                	TransformToYUVPlaner(&picture, pOutBufHdr->pBuffer,0);
            	}
            	else
            	{
                    //ALOGV("(omx_vdec, f:%s, l:%d) m_useAndroidBuffer==OMX_TRUE", __FUNCTION__, __LINE__);
                  #if (OPEN_STATISTICS)
                ...
                if(0 == nRetSemGetValue)
                {
                    ...
                    {
                        //ALOGV("(f:%s, l:%d) Be careful, sema frame_output[%d]!=0", __FUNCTION__, __LINE__, nSemVal);
                    }
                }
                else
                {
                ...
                if(eos && nVdrvNotifyEosFlag)
                {
                	if(!pSelf->m_decoder->picture_ready(pSelf->m_decoder))
                	{
                        ALOGV(" set up eos flag to picture.");
            ...
            else
            {
                if(nVdrvNotifyEosFlag)
                {
                	...
                	while(pSelf->m_sOutBufList.nSizeOfList > 0)
                	{
                        ...
                        if(pSelf->m_sOutBufList.nSizeOfList == 0)
                        {
                            ALOGV("(f:%s, l:%d), EOS msg send with FillBufferDone", __FUNCTION__, __LINE__);
                    		pOutBufHdr->nFlags |= OMX_BUFFERFLAG_EOS;
                        }
                		pSelf->m_Callbacks.FillBufferDone(&pSelf->m_cmp, pSelf->m_pAppData, pOutBufHdr);
                		pOutBufHdr = NULL;
...
//                tm1 = OMX_GetNowUs();
                ...
                {
//                    tm2 = OMX_GetNowUs();
//                    ALOGV("(f:%s, l:%d) no vdeclib frame, wait 20ms for msg, interval=[%lld]ms", __FUNCTION__, __LINE__, (tm2-tm1)/1000);
                }
                else
                {
//                    tm3 = OMX_GetNowUs();
//                    ALOGV("(f:%s, l:%d) no vdeclib frame, wait <20ms for msg coming, interval=[%lld]ms", __FUNCTION__, __LINE__, (tm3-tm1)/1000);
                }
            }
        }
    }
...
static void* ComponentVdrvThread(void* pThreadData)
{
    ...
    while (1)
    {
        ...
        if (FD_ISSET(pSelf->m_vdrv_cmdpipe[0], &rfds))
        {
            ...
            ALOGV("(f:%s, l:%d) vdrvThread receive cmd[0x%x]", __FUNCTION__, __LINE__, cmd);
            // State transition command
	        if (cmd == OMX_VdrvCommand_PrepareVdecLib)  //now omx_vdec's m_state = OMX_StateLoaded, OMX_StateLoaded->OMX_StateIdle
	        {
	        	ALOGV("(f:%s, l:%d)(OMX_VdrvCommand_PrepareVdecLib)", __FUNCTION__, __LINE__);
                // If the parameter states a transition to the same state
                // raise a same state transition error.
                if (pSelf->m_state != OMX_StateLoaded)
                {
...
//                ALOGV("set max_output width[%d], height[%d]", 2048, 1288);
...
//        		pSelf->m_decoder->ioctrl(pSelf->m_decoder, CEDARV_COMMAND_SET_MAX_OUTPUT_HEIGHT, 1288); //2304, 1288
	        ...
	        {
                ALOGV("(f:%s, l:%d)(OMX_VdrvCommand_CloseVdecLib)", __FUNCTION__, __LINE__);
	        ...
	        {
                ALOGV("(f:%s, l:%d)(OMX_VdrvCommand_FlushVdecLib)", __FUNCTION__, __LINE__);
                if(pSelf->m_decoder)
                {
    				if (ve_mutex_lock(&pSelf->m_cedarv_req_ctx) < 0) {
	        ...
	        {
                ALOGV("(f:%s, l:%d)(OMX_VdrvCommand_Stop)", __FUNCTION__, __LINE__);
                nStopFlag = 1;
	        }
	        else if (cmd == OMX_VdrvCommand_EndOfStream)
	        {
                ALOGV("(f:%s, l:%d)(OMX_VdrvCommand_EndOfStream)", __FUNCTION__, __LINE__);
                nEosFlag = OMX_TRUE;
	        }
	        else
	        {
...
__EXECUTE_CMD_DONE:
        ...
        if(nStopFlag)
        {
            ALOGV("vdrvThread detect nStopFlag[%d], exit!", nStopFlag);
            goto EXIT;
        }
        // Buffer processing
        // Only happens when the component is in executing state.
        if (pSelf->m_state == OMX_StateExecuting || pSelf->m_state == OMX_StatePause)
        {
            ...
            {
                ALOGV("decode ret[%d], ignore? why?", decodeResult);
            }
        }
        else
        {
...
//            tm1 = OMX_GetNowUs();
            ...
            {
//                tm2 = OMX_GetNowUs();
//                ALOGV("(f:%s, l:%d) vdrvThread m_state[0x%x] is not OMX_StateExecuting or OMX_StatePause, wait 20ms for msg, interval=[%lld]ms", __FUNCTION__, __LINE__, pSelf->m_state, (tm2-tm1)/1000);
            }
            else
            {
//                tm3 = OMX_GetNowUs();
//                ALOGV("(f:%s, l:%d) vdrvThread m_state[0x%x] is not OMX_StateExecuting or OMX_StatePause, wait <20ms for msg coming, interval=[%lld]ms", __FUNCTION__, __LINE__, pSelf->m_state, (tm3-tm1)/1000);
            }
        }
    }
