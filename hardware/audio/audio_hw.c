...
static void tinymix_list_controls(struct mixer *mixer)
{
    ...
    ALOGD("Number of controls: %d", num_ctls);

    ALOGD("ctl\ttype\tnum\t%-40s value", "name");
    ...
    while(PcmManager->DataLen < uGetLen)
    {
        ...
        if(timeout > max_wait_count) {
        	if (PcmManager->DataLen < uGetLen) {
        		...
        		ALOGW("fill with dc size:%d",uGetLen - PcmManager->DataLen);
        	}
        	break;
        }
    }
...
static int start_output_stream(struct sunxi_stream_out *out)
{
	...
	for (index = 0; index < MAX_AUDIO_DEVICES; index++)
	{
			...
			&& adev->dev_manager[index].flag_out_active)
		{
			...
			out->multi_pcm[card] = pcm_open(card, port, PCM_OUT | PCM_MMAP | PCM_NOIRQ, &out->multi_config[card]);
...
static int start_input_stream(struct sunxi_stream_in *in)
{
	...
	if (adev->active_output)
	{
		...
		ALOGV("there is an active output stream, rate: %d, force capture rate: %d", out_rate, in_ajust_rate);
	}
                                         ...
                                         const struct audio_config *config)
{
    ...
    if (check_input_parameters(config->sample_rate, config->format, channelCount) != 0)
        return 0;

    return get_input_buffer_size(config->sample_rate, config->format, channelCount);
}
                                  ...
                                  audio_stream_in_t **stream_in)
{
	...
	ALOGV("+++++++++++++++ adev_open_input_stream: req_sample_rate: %d, fmt: %x, channel_count: %d",
		config->sample_rate, config->format, channel_count);
	...
	if (devices == AUDIO_DEVICE_IN_REMOTE_SUBMIX) {
		ALOGV("to malloc PcmManagerBuffer: Buffer_size: %d", WD_BUFFER_SIZE);
                     ...
                     hw_device_t** device)
{
	...
	{
		free(adev);
		ALOGE("Unable to open PA controller, aborting.");
        return -EINVAL;
	}
	...
	if (!adev->mixer_ctls.playback_pamute_switch) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_PLAYBACK_PAMUTE_SWITCH);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.playback_mixpas) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_PLAYBACK_MIXPAS);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.playback_dacpas) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_PLAYBACK_DACPAS);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.mic_output_mix) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_MIC_OUTPUT_MIX);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.ldac_right_mixer) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_LDAC_RIGHT_MIXER);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.rdac_right_mixer) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_RDAC_RIGHT_MIXER);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.ldac_left_mixer) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_LDAC_LEFT_MIXER);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.fmr_switch) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_FMR_SWITCH);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.fml_switch) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_FML_SWITCH);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.liner_switch) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_LINER_SWITCH);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.linel_switch) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_LINEL_SWITCH);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.mic_output_volume) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_MIC_OUTPUT_VOLUME);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.fm_output_volume) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_FM_OUTPUT_VOLUME);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.line_output_volume) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_LINE_OUTPUT_VOLUME);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.mix_enable) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_MIX_ENABLE);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.dacalen_enable) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_DACALEN_ENABLE);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.dacaren_enable) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_DACAREN_ENABLE);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.pa_enable) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_PA_ENABLE);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.mic1outn_enable) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_MIC1OUTN_ENABLE);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.linein_pam_volume) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_LINEIN_APM_VOLUME);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.line_in_r_function_define) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_LINE_IN_R_FUNCTION_DEFINE);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.adc_input_source) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_ADC_INPUT_SOURCE);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.capture_volume) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_CAPTURE_VOLUME);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.mic2_gain_volume) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_MIC2_GAIN_VOLUME);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.mic1_gain_volume) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_MIC1_GAIN_VOLUME);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.vmic_enable) {
			ALOGE("Unable to find '%s' mixer control",MIXER_VMIC_ENABLE);
			goto error_out;
	}
	...
	if (!adev->mixer_ctls.mic2_amplifier_enable) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_MIC2_AMPLIFIER_ENABLE);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.mic1_amplifier_enable) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_MIC1_AMPLIFIER_ENABLE);
	        goto error_out;
	}
	...
	if (!adev->mixer_ctls.adcl_enable) {
	    ALOGE("Unable to find '%s' mixer control",MIXER_ADCL_ENABLE);
	    goto error_out;
	}
	...
	if (!adev->mixer_ctls.adcr_enable) {
		ALOGE("Unable to find '%s' mixer control",MIXER_ADCR_ENABLE);
		goto error_out;
	}
